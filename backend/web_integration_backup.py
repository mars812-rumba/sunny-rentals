#!/usr/bin/env python3log_dialog_event
# -*- coding: utf-8 -*-
...
def log_dialog_event(user_id: int, action: str, timestamp : str):
    """
...
async def api_send_claude_message(request: Request):
    """Отправляет сообщение в диалог Claude"""
    try:
        ...
        if user_conversations[user_id].get("status") != "active":
            return JSONResponse(content={"status": "error", "message": "Диалог не активен"})
...
async def api_get_dialog_events(user_id: int, limit: int = 50):
    """Получить события диалога для пользователя"""
    try:
        ...
        print(f"❌ Error in api_get_dialog_events: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/crm/dialog/{user_id}/mark-read")
async def api_mark_dialog_read(user_id: int):
    """Сбросить флаг новых сообщений"""
    try:
        update_dialog_status(
            user_id=user_id,
            has_new_messages=False
        )
        log_dialog_event(user_id, action="messages_marked_read",timestamp=datetime.now().isoformat())
        
        return JSONResponse(content={"status": "ok"})
    except Exception as e:
        print(f"❌ Error in api_mark_dialog_read: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/claude/status/{user_id}")
async def api_get_claude_status(user_id: int):
    """Получает статус диалога Claude для пользователя"""
    try:
        user_id = int(user_id)
        
        # Получаем статус диалога из базы данных
        ...
        return JSONResponse(content={
            ...
            "data": {
                "user_id": user_id,
                "claude_status": dialog_status.get("claude", {}).get("status", "stopped"),
                "enabled": dialog_status.get("claude", {}).get("enabled", False),
                "active": dialog_status.get("active", False),
                "message_count": dialog_status.get("message_count", 0),
                "has_new_messages": dialog_status.get("has_new_messages", False)
            }
        })
            
    except Exception as e:
        print(f"❌ Error in api_get_claude_status: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/claude/active_dialogs")
async def api_get_active_dialogs():
    """Получает список всех активных диалогов Claude"""
    try:
        active_dialogs = []
        ...
        for user_id, conversation in user_conversations.items():
            if conversation.get("status") == "active":
                active_dialogs.append({
                    ...
                    "history_length": len(conversation.get("history", [])),
                    "filters": conversation.get("filters", {}),
                    "claude_initiated": user_sessions.get(user_id, {}).get("claude_initiated", False)
                })
        ...
        print(f"❌ Error in api_get_active_dialogs: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def notify_telegram_bot_about_filters(user_id: int, filters: dict, username: str = None) -> bool:
    ...
    try:
        ...
        # ✅ ОБНОВЛЯЕМ СТАТУС ДИАЛОГА: устанавливаем активный диалог и добавляем счётчик сообщений
        update_dialog_status(
            user_id=user_id,
            active=True,
...
def notify_telegram_bot_about_webapp_opened(user_id: int, username: str = None) -> bool:
    ...
    try:
        ...
        # ✅ ОБНОВЛЯЕМ СТАТУС ДИАЛОГА: устанавливаем активный диалог
        update_dialog_status(
            user_id=user_id,
            active=True,
            username=username
...
def notify_telegram_bot_about_booking(booking_id: str, user_id: int, form_data: dict, username: str = None):
    ...
    try:
        ...
        # ✅ ОБНОВЛЯЕМ СТАТУС ДИАЛОГА: добавляем счётчик сообщений и обновляем информацию о бронировании
        update_dialog_status(
            user_id=user_id,
            active=True,
            last_message_at=datetime.now().isoformat(),
...
def get_crm_users(status: str, period: str = "week"):
    try:
        ...
        # Теперь мы увидим более детальную ошибку, если она случится
        return {"status": "error", "message": str(e)}
...
async def send_message_to_user(msg_request: SendMessageRequest):
    ...
    try:
        ...
        log_dialog_event(msg_request.user_id, action="manager_message_sent",
            timestamp=datetime.utcnow().isoformat()
        )
...
async def receive_message_from_bot(request: Request):
    ...
    try:
        ...
        log_dialog_event(user_id,action="receive_message_from_bot",timestamp=datetime.utcnow().isoformat())
