...
const CarCard = ({ car, rentalDays, onBooking, isSubmitting }: CarCardProps) => {
  ...
  const getSeason = (date: Date) => {
    console.log('CarCard.tsx getSeason date:', date);
    if (!date || isNaN(date.getTime())) {
      console.log('CarCard.tsx getSeason date is invalid, using new Date()');
      date = new Date();
    }
    const month = date.getMonth() + 1; // 1-12
    // Высокий сезон: ноябрь-март (11, 12, 1, 2, 3)
    if (month >= 11 || month <= 3) {
      return 'high_season';
    }
    // Низкий сезон: апрель-октябрь (4-10)
    return 'low_season';
  };

  const getPriceForPeriod = (pricing: any, days: number) => {
    console.log('CarCard.tsx getPriceForPeriod days:', days);
    const today = new Date();
    console.log('CarCard.tsx getPriceForPeriod today:', today);
  ...
  const dailyPrice = getPriceForPeriod(car.pricing, rentalDays > 0 ? rentalDays : 1);
            ...
            <div className="flex items-end justify-between">
              <div>
                <div className="text-2xl font-bold text-primary">
                  {rentalDays > 0 ? `${totalPrice.toLocaleString()}฿` : `${dailyPrice.toLocaleString()}฿`}
                </div>
                <div className="text-sm text-muted-foreground">
                  {rentalDays > 0 
                    ? `${dailyPrice}${t('per_day')} • ${t('days', { rentalDays })}` 
                    : `${t('price_per_day_from')} ${dailyPrice}${t('per_day')}`
                  }
                </div>
              </div>
            </div>
...
<Button 
  ...
  className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-semibold py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
>
  {isSubmitting ? (
    <>
